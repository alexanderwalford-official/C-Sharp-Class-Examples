...
public class SimplePlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C))
        {
            if (iscrouched)
            {
                playerCamera.transform.position = new Vector3(playerCamera.transform.position.x, playerCamera.transform.position.y + crouchdistance, playerCamera.transform.position.z);
                iscrouched = false;
